...
import { CEOData } from "@/types/chat";
...
export const CEOTable = ({ ceos, onFindJobs, onFindInvestors }: CEOTableProps) => {
  const [selectedCompany, setSelectedCompany] = useState<CEOData | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [sortField, setSortField] = useState<keyof CEOData | null | 'rank'>(null);
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');

  const handleRowClick = (company: CEOData) => {
    setSelectedCompany(company);
    setIsModalOpen(true);
  };

  const downloadAsExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(ceos);
  ...
  const handleSort = (field: keyof CEOData | 'rank') => {
    if (sortField === field) {
  ...
  const sortedCompanies = [...ceos].sort((a, b) => {
    ...
    let fieldA = sortField === 'leadScores' ? a.leadScores?.rank : a[sortField];
    let fieldB = sortField === 'leadScores' ? b.leadScores?.rank : b[sortField];
  ...
  const SortIcon = ({ field }: { field: keyof CEOData | 'rank' }) => (
  ...
  const renderSortableHeader = (field: keyof CEOData | 'rank', label: string) => (
    <th 
