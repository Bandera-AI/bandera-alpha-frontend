...
import { JobsTable } from "@/components/chat/JobsTable";
import { InvestorsTable } from "@/components/chat/InvestorsTable";
import { JobData } from "@/types/chat";
import { InvestorData } from "@/types/chat";
import { RemoveContactModal } from "./RemoveContactModal";

// Sample data for whatsapp contacts
const WHATSAPP_CONTACTS: Contact[] = [
  {
    id: "e1",
    name: "Sarah Johnson",
...
export const WhatsappInbox: React.FC<WhatsappInbox> = ({ contacts: propContacts }) => {
  // Use provided contacts or fallback to sample data
  const [contacts, setContacts] = useState<Contact[]>(propContacts || []);
  const [threads, setThreads] = useState<Thread[]>(WHATSAPP_THREADS);
